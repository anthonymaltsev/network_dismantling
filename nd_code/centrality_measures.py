...
# draw_greedy_algorithm(heuristic, G, c=.2)
...
def greedy_algorithm(heuristic, _G, c=3, timeout=300):
  start = time.time()
...
def draw_greedy_algorithm(heuristic, G, c=0.2):
  ...
  S, _ = greedy_algorithm(heuristic, G, c=c)
  for node in S:
    colors[node] = "red"
...
def second_eval_estimate(full_G) :
  ...
  L = nx.laplacian_matrix(full_G)
  try :
    v_2 = nx.fiedler_vector(full_G)
  except :
    return 0
