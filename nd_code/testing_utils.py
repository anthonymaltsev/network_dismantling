...
import numpy as np
import random
from nd_code.centrality_measures import greedy_algorithm
from nd_code.gnd import gnd
...
def test_heuristics(G, heuristics, timeout=300, name="no name provided", c=3):
  results = []
  print("Graph:", name)
  ...
  for heuristic in heuristics:
    ...
    h = heuristic[1]
    print("Testing", name)
    results.append((name, greedy_algorithm(h, G, c=c, timeout=timeout)))
    print("")
...
def test_graphs(graphs, g_type, heuristics, timeout=300, c=3, gnd=False):
  ...
  for graph in graphs:
    ...
    if gnd:
      ...
      graph_results.append(test_heuristics(G, heuristics, timeout=timeout, name=name, c=c))
    print("~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    print("")
